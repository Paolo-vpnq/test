...
async function submitObservation() {
  ...
  if (navigator.onLine && CONFIG.ENDPOINT_URL) {
    ...
    // Show persistent notification — keeps SW alive for background sync
    if ('Notification' in window && Notification.permission === 'granted') {
      getAllPending().then(pending => showPendingNotification(pending.length)).catch(() => {});
    } else if ('Notification' in window && Notification.permission !== 'denied') {
      // iOS: request permission on user gesture (submit tap) — iOS requires gesture for permission
...
async function init() {
  ...
  if ('serviceWorker' in navigator) {
    ...
    // iOS: reload when a new SW takes control (handles iOS PWA lifecycle)
  ...
  // iOS: check for SW updates on app resume (iOS doesn't re-fire DOMContentLoaded)
  document.addEventListener('visibilitychange', () => {
    if (document.visibilityState === 'visible' && 'serviceWorker' in navigator) {
      navigator.serviceWorker.ready.then(reg => reg.update().catch(() => {}));
    }
  });
  // iOS: check for SW updates when restored from bfcache
